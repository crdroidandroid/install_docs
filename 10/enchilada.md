...
_(Guide adapted from the Evolution-X installation guide by @jabashque and @AnierinBliss)_
...
- Ensure you're running OxygenOS 11.1.2.2 firmware on your current slot. Instructions for how to do this are beyond the scope of this guide, but you can always use MSMTool to go back to stock OxygenOS with locked bootloader, and start updating via OTA until you're up to date, or follow the firmware updating guides for enchilada or fajita on the LineageOS wiki.
  ...
  If you do not have that firmware installed, grab it for your device from one of the following links:

  **[enchilada (OnePlus 6)](https://oxygenos.oneplus.net/OnePlus6Oxygen_22.J.62_OTA_0620_all_2111252336_287bcb1636d743d3.zip)**

  **[fajita (OnePlus 6T)](https://oxygenos.oneplus.net/OnePlus6TOxygen_34.J.62_OTA_0620_all_2111252336_f6eda340d7af4e3e.zip)**
...
- Download the LineageOS copy-partitions script from here:
  ...
  **[copy-partitions flashable zip](https://mirrorbits.lineageos.org/tools/copy-partitions-20220613-signed.zip)**

- Download the needed files for you device from here (will be updated to sourceforge once I'm ready to take 14.0 official):

  **[enchilada (OnePlus 6)](https://sourceforge.net/projects/crdroid/files/enchilada/10.x/)**

  **[fajita (OnePlus 6T)](https://sourceforge.net/projects/crdroid/files/fajita/10.x/)**
...
5. Run the following to flash the retrofit dynamic partitions boot/recovery image, dtbo image, and the empty vbmeta to the current slot (replacing `crDroidAndroid-14.0-xxx_thing.img` with the actual filename that you downloaded for each file):
    ```
...
9. Initialize the retrofit super partitions for the current slot:
  ...
  ** WARNING: If fastboot returns some error message about not recognizing `wipe-super` or prints a long help message instead, then that means you are running a very old version of the fastboot executable. Please go back and download the latest version of platform-tools, as mentioned in the [Required Files](#required-files) section. **
...
17. Choose `Reboot system now` and remember: **𓂀 We love you 3000!**
...
6. On the phone, choose `Reboot system now` and remember: **𓂀 We love you 3000!**
